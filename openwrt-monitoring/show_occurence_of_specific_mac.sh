...
if [ -z "$MAC" ]; then
	echo "Usage: $0 <macaddress>	# 112233445566 or 11:22:33:44:55:66 or part: 22:33:44"
	exit 1
else
...
for NETWORK in $( list_networks ); do {
	...
	if [ -e "$FILE" ]; then
		...
		# only a part:
		ls -l /var/www/networks/$NETWORK/meshrdf/recent/*${MAC}* 2>/dev/null && {
			echo "(in network $NETWORK)"
		}
	fi
} done
